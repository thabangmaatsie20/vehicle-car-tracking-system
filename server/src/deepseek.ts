...
export async function getMovementInsights(opts: InsightOptions): Promise<string> {
  const { deviceId, fromTs, toTs, limit = 200, model = process.env.DEEPSEEK_MODEL || 'deepseek-chat', apiKey } = opts;

  const query: any = { deviceId };
  if (fromTs || toTs) {
    query.createdAt = {};
    if (fromTs) query.createdAt.$gte = new Date(fromTs);
    if (toTs) query.createdAt.$lte = new Date(toTs);
  }

  const readings = await ReadingModel.find(query, { gps: 1, createdAt: 1 })
    .sort({ createdAt: -1 })
    .limit(limit)
    .lean();
